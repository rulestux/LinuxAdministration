...
### cat *file*
- concatena e retorna o conteúdo de arquivos de texto simples.
...
### cut
- corta e exibe partes de um arquivo de texto, separando-o em colunas, usando um caractere fornecido como referência ```-d'caractere'```, seguido da posição \(campos) dos elementos a serem exibidos com ```-fnúmero,número,número```:
```bash
$ cut -d; -f1,6,7 /caminho/do/arquivo
```
- ou em um intervalo de caracteres com ```-cnúmero-número```:
```bash
